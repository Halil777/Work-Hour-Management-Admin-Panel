...
export default function TopMonthlyWorkers() {
  const theme = useTheme();

  const { data, isLoading } = useQuery({
    queryKey: ["top-monthly-workers"],
    queryFn: async () => {
      const res = await api.get("/admin/stats/top-monthly");
      return res.data as {
      ...
      {isLoading ? (
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data || []} layout="vertical">
            ...
            />
            <Tooltip />
            <Bar dataKey="totalHours" fill="#4caf50" radius={[0, 6, 6, 0]} />
          </BarChart>
        </ResponsiveContainer>
      )}
    </Paper>
